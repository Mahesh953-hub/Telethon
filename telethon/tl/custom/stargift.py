...
class StarGift:
    ...
    async def upgrade(self, keep_original_details: Optional[bool] = None):
        """
        Upgrades this saved gift via :tl:`payments.UpgradeStarGiftRequest`.

        Only works when the upgrade is free or prepaid
        (``prepaid_upgrade_hash``/``upgrade_stars`` covered); paid upgrades
        must go through an invoice + payment flow.
        """
        from ..functions.payments import UpgradeStarGiftRequest
        from ..types import InputSavedStarGiftUser

        return await self._client(UpgradeStarGiftRequest(
            stargift=InputSavedStarGiftUser(msg_id=self.msg_id),
